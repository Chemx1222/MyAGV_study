...
# ROI가 프레임 크기를 벗어나지 않도록 검증
roi_x = max(0, min(roi_x, frame_width - 1))
roi_y = max(0, min(roi_y, frame_height - 1))
roi_width = max(1, min(roi_width, frame_width - roi_x))
roi_height = max(1, min(roi_height, frame_height - roi_y))
...
while True:
    ...
    frame[roi_y:roi_y+roi_height, roi_x:roi_x+roi_width] = combined
